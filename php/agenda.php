			...
			function agenda($msql,$tipo,$fecha){
			  ...
			  while($t<=strtotime($final))
			  {
			    ...
			    for($j=0;$j<$nocons;$j++)
					 ...
					 $a[]=array("vhoras"=>sprintf("%s.%d",date("H:i",$t),$j+1),"paciente"=>"","nombres"=>"","procedimiento"=>"","confirmo"=>"","color"=>"","cara"=>"","abono"=>"","conhistoria"=>"","identificacion"=>"","citasind"=>"");
				
				
				$t=strtotime(SumarMinutosFechaStr(date("d/m/Y H:i:s",$t), $duracita));
				
			  }
			  $sql="select concat_ws('.',time_format(vhoras,'%H:%i'),consultorio) as vhoras,citas.paciente,nombres,procedimientos.nombre as procedimiento,confirmo,if( asistio='N' and enatencion='N' and  conhistoria='N',color,if( asistio='S' and enatencion='N' and conhistoria='N','50;140;190',if (asistio='S' and enatencion='S' and conhistoria='N','83;125;79','00;00;00'))) as color,pagos.tipest,abono,citas.duracion,consultorio,conhistoria,paciente.identificacion,if((telefono_movil='')or(telefono_movil is null),telefono_residencia1,telefono_residencia2) as telefono,citas.ind as citasind,procedimiento as codproc,'C' as queagenda from citas";
			  ...
			  $sql.=" UNION ";
			  $sql.=" select concat_ws('.',time_format(vhoras,'%H:%i'),consultorio) as vhoras,cppre.paciente,nombres,procedimientos.nombre as procedimiento,confirmo,if( asistio='N' and enatencion='N' and  conhistoria='N',color,if( asistio='S' and enatencion='N' and conhistoria='N','50;140;190',if (asistio='S' and enatencion='S' and conhistoria='N','83;125;79','00;00;00'))) as color,'AZUL',abono,cppre.duracion,consultorio,conhistoria,cppredata.identificacion,if((telefono_movil='')or(telefono_movil is null),telefono_residencia1,telefono_residencia2) as telefono,cppre.ind,procedimiento,'P' from cppre";
			  ...
			  if($result = mysql_query($sql,$msql))
			  while($fila=mysql_fetch_array($result))
			  {
				  ...
				  if ($a[$i]["vhoras"]==$fila["vhoras"]){
					...
					$b=array("vhoras"=>$fila['vhoras'],"paciente"=>$fila['paciente'],"nombres"=>utf8_encode($fila['nombres']),"procedimiento"=>utf8_encode($fila['procedimiento']),"confirmo"=>$fila['confirmo'],"color"=>$color,"cara"=>$fila['tipest'],"abono"=>$fila['abono'],"conhistoria"=>$fila["conhistoria"],"identificacion"=>$fila["identificacion"],"telefono"=>$fila["telefono"],"citasind"=>$fila["citasind"],"codproc"=>$fila["codproc"],"queagenda"=>$fila["queagenda"]);  
					$a[$i]=$b;
					...
					for($j=1;$j<=$repite-1;$j++){
						...
						$b=array("vhoras"=>$vhoras,"paciente"=>$fila['paciente'],"nombres"=>utf8_encode($fila['nombres']),"procedimiento"=>utf8_encode($fila['procedimiento']),"confirmo"=>$fila['confirmo'],"color"=>$color,"cara"=>$fila['tipest'],"abono"=>$fila['abono'],"conhistoria"=>$fila["conhistoria"],"identificacion"=>$fila["identificacion"],"telefono"=>$fila["telefono"],"citasind"=>$fila["citasind"],"codproc"=>$fila["codproc"],"queagenda"=>$fila["queagenda"]);  
						$a[$i]=$b;
					}
				  }
				  ...
				  $a=array("Error"=>"Error","Mensaje"=>mysql_error($msql),"sql"=>$sql);
			  }
